...
namespace rsc { namespace patterns {
  ...
  public:
    ...
    create_base(const Key&                 key,
		const runtime::Properties& properties_
		= runtime::Properties()) throw (NoSuchImpl,
						ConstructError) = 0;
  };
  ...
  public:
    ...
    typedef boost::function1<interface_type*,
                             const runtime::Properties&> create_function;
  ...
  template <typename Key,
	    typename Interface>
  Factory<Key, Interface>::impl_map_proxy::impl_map_proxy(Factory<Key, Interface>& owner)
  ...
  template <typename Key,
	    typename Interface>
  void
  Factory<Key, Interface>::impl_map_proxy::register_(const key_type&        key,
						     const create_function& create_function_) throw (std::invalid_argument) {
  ...
  template <typename Key,
	    typename Interface>
  void
  Factory<Key, Interface>::impl_map_proxy::unregister(const key_type& key) throw (NoSuchImpl) {
  ...
  template <typename Key,
	    typename Interface>
  void
  Factory<Key, Interface>::register_(const Key&             key,
				     const create_function& create_function_) throw (std::invalid_argument) {
    //
  ...
  template <typename Key,
	    typename Interface>
  void
  Factory<Key, Interface>::unregister(const Key& key) throw (NoSuchImpl) {
    //
    typename impl_map::iterator it;
    if ((it = this->impl_map_.find(key)) == this->impl_map_.end()) {
      throw NoSuchImpl(runtime::type_string("no implementation found for specified key `%1%'",
					    "no implementation found for specified key",
					    key));
    }
  ...
  template <typename Key,
	    typename Interface>
  typename FactoryBase<Key>::type_and_storage
  Factory<Key, Interface>::create_base(const Key&                 key,
  ...
  Factory<Key, Interface>::create_inst(const Key&                 key,
    ...
    try {
      ...
      throw ConstructError(runtime::type_string("could not construct implementation instance for key `%1%'",
						"could not construct implementation instance",
						key));
    }
  ...
  operator<<(std::basic_ostream<Ch, Tr>&    stream,
    ...
    typedef typename Factory<Key, Interface>::impl_map_proxy impl_map_proxy_type;
    ...
    for (typename impl_map_proxy_type::const_iterator it = impls.begin();
	 it != impls.end(); ++it)
